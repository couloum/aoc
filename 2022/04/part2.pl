...
while (<FILE>) {
  ...
  if ($_ =~ /(\d+)-(\d+),(\d+)-(\d+)/) {
      ...
      if ( ($e1_start >= $e2_start and $e1_start <= $e2_end) or
           ($e1_end   >= $e2_start and $e1_end   <= $e2_end) or
           ($e2_start >= $e1_start and $e2_start <= $e1_end) or
           ($e2_end   >= $e1_start and $e2_end   <= $e1_end)) {
         debug("Found one overlap: $_\n");
         $result++;
       } else {
         debug("Not overlaping: $_\n");
       }
  }
}
