...
sub which_shape {
  ...
  my $rank = ord($opp_shape) - 65; # 0 = Rock, 1 = Paper, 2 = Scissors
  ...
  $my_shape = chr(65 + ($rank + $mod->{$condition}) % 3);

  debug(3, "Opp shape: $opp_shape // condition: $condition // my_shape: $my_shape");
...
while (<FILE>) {
  ...
  if ($_ =~ /^([ABC]) ([XYZ])/) {
    ...
    # get the shape I must play
    my $b = which_shape($a, $c);
    ...
    debug(3, "$a $b $c || $elves_score $my_score\n");
    ...
    debug("$a $b $c || $elves_score $my_score || $total_score\n");
