...
def get_result(raw_data):
    ...
    accessibe_rolls_xy = ["fakedata"]
    step = 1
    while len(accessibe_rolls_xy) > 0:
        accessibe_rolls_xy = []
        _log("Identifying accessible rolls (step %d)" % (step))
        for y in range(len(map)):
            for x in range(len(map[y])):
                if map[y][x] == ".":
                ...
                if get_num_adj_rolls(map, y, x) < 4:
                    _log("Found accessible roll on position x=%d,y=%d" % (x, y), 2)
                    accessibe_rolls_xy.append(yx_to_s(y,x))
                    total += 1
        _log("Found %d accessible rolls during step %d" % (len(accessibe_rolls_xy), step))
        ...
        # Now remove the accessible rolls
        for yx in accessibe_rolls_xy:
            (y,x) = s_to_yx(yx)
            map[y][x] = "."
