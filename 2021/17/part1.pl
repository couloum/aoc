...
sub get_x_velocities {
  ...
  for (my $x_vel = 1; $x_vel < $x_range; $x_vel++) {
    ...
    $step_max = $x_vel_cur == 0 ? 500 : $nb_steps;
...
sub get_xy_velocities {
  ...
  my @xy_vel = ();
  for (my $y_vel = -30; $y_vel < 500; $y_vel++) {
...
debug "Find maximum Y value\n";
my $y_max = 0;
foreach my $xy_vel_info (@xy_velocities) {
  my ($x_vel, $y_vel, $steps) = split(',', $xy_vel_info);

  my $y = get_highest_point($y_vel);
  if ($y_max < $y) {
    $y_max = $y;
    debug "Found maximum Y value $y for x_vel=$x_vel, y_vel=$y_vel\n";
  }
}
