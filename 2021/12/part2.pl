...
sub generate_paths {
  ...
  # Allow it once: when done, add a '*' (star) in the path so we know a small cave has already been visited twice.
  ...
  while (!$flag_end) {
    ...
    foreach my $path (@paths) {
      ...
      foreach my $connection (keys %$connections) {
        ...
        my ($left, @right);
        if ($last_cave eq $start) {
          $left = $start;
          @right = ($end);
        } elsif ($last_cave eq $end) {
          $left = $end;
          @right = ($start);
        }

        # Check if right end is a small cave
        if ($right[0] =~ /^[a-z]+$/) {
          # Check if we already visited it
          if (grep(/^$right[0]$/, @path_save)) {
            # Check if we already visited twice a small cave
            if (grep(/^\*$/, @path_save)) {
              # then we ignore this connection
              next;
            } else {
              # Otherwise, we consider this connection, but add a '*' to mark that we will visit twice a small cave
              @right = ('*', $right[0]);
            }
          }
        }
