...
def get_result(raw_data):
    ...
    print_robots_on_map([r['p'] for r in data], map_size)
    end_positions = []
    for robot in data:
        end_pos = get_position_after(robot['p'], robot['v'], map_size, iterations)
        end_positions.append(end_pos)
        _log("End position for robot %s after %d seconds: (%s)" % (robot, iterations, end_pos))
    ...
    total = 1
    for (q,val) in nb_per_quarter.items():
        _log("%d robots in quarter %s" % (val, q))
        total *= val
