...
class Computer:
    ...
    def process_next(self):
        ...
        match opcode:
            ...
            case 5: #out
                ...
                _log("Instruction out: append (%d %% 8) = %d to the output" % (combo_operande, combo_operande%8))
    ...
    def process_instructions(self):
        _log("Start processing all instructions")
        i = 0
        while self.process_next():
            i += 1
            continue
        _log("End processing all instructions after %d rounds" % (i))
        _log("Registers: %s" % (self.registers))
        _log("Output: %s" % (self.output))
        return self.output
