...
def get_result(raw_data):
    ...
    unique_trails = []
    for trail in trails:
        trail_start_end = "%s-%s" % (trail[0], trail[-1])
        if not trail_start_end in unique_trails:
            _log("Found a trail starting at (%s) and ending at (%s)" % (trail[0], trail[-1]))
            unique_trails.append(trail_start_end)
