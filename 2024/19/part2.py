...
def check_design(towels_dict, design, patterns = []):
    """
    ...
    if len(design) == 0:
        _log("1 full match found: %s" % (patterns), 2)
        return 1
    
    # Get maximum pattern size
    max_size = min(len(design), max(towels_dict.keys()))

    total_match = 0
    for i in range(1, max_size+1):
        pattern = design[0:i]
        _log("Looking for a towel matching pattern %s" % (pattern), 2)
        if pattern in towels_dict[i]:
            _log("Pattern %s found in design %s" % (pattern, design), 2)
            patterns.append(pattern)
            total_match += check_design(towels_dict, design[i:], patterns.copy())
...
def get_result(raw_data):
    ...
    # Redesign towels to group every pattern by length
    max_length = max([len(x) for x in towels])
    towels_dict = dict()
    for i in range(1, max_length+1):
        towels_dict[i] = []
    for towel in towels:
        towels_dict[len(towel)].append(towel)
    ...
    for design in designs:
        _log("Checking design %s" % (design))
        tmp = check_design(towels_dict, design)
        if tmp > 0:
            _log("[OK] Possible to create design %s with %d different combinations" % (design, tmp))
            total += tmp
        else:
