...
def get_result(raw_data):
    ...
    Detect updates where ordered version is different from original version (is an invalid update, but is
    what we want to detect).
    For each, take the middle number and add it to total
    """
    total = 0
    (data_rules, data_updates) = read_data_custom(raw_data)

    #For each update, sort it using custom sort function
    #If result after "sort" is different than before, then update is the one we retain
    for update in data_updates:
        ...
        if update_arr != ordered_update:
            value = int(ordered_update[int(len(ordered_update)/2)]) # Get value from middle number
            total += value
            _log("[OK] %s is an invalid update. add %d. Total=%d" % (update_arr, value, total))
        else:
            _log("[!!] %s is a valid update" % (update_arr))
